          ...
          {scans.length > 0 ? (
            scans.map((scan) => (
              <motion.div
                ...
                className="overflow-hidden rounded-xl border border-zinc-700 bg-zinc-800/80 shadow-lg"
              >
                <div className="relative h-48 overflow-hidden rounded-t-xl bg-zinc-800">
                  ...
                  />
                  <div className="absolute right-2 top-2 rounded-lg bg-zinc-900/90 px-2 py-1 text-lg font-bold text-teal-400">
                    {scan.overallScore}
                  </div>
                </div>
                <div className="border-t border-zinc-700 px-4 py-3">
                  ...
                  {scan.aiSummary && (
                    <p className="mt-2 text-xs text-zinc-400 line-clamp-2">
                      {scan.aiSummary.length > 80
                        ? `${scan.aiSummary.slice(0, 80)}…`
                        : scan.aiSummary}
                    </p>
                  )}
                </div>
        ...
        <div className="space-y-3">
          {reports.map((report) => (
            <button
              key={report.title}
              type="button"
              className="group flex w-full items-center justify-between rounded-xl border border-zinc-700 bg-zinc-800/50 px-4 py-3 transition-colors hover:border-teal-400/30 hover:bg-zinc-800"
            >
              ...
              <Download className="h-5 w-5 shrink-0 text-zinc-500 transition-colors group-hover:text-teal-400" />
            </button>
          ))}
        </div>
      </motion.section>
